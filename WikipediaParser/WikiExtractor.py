...
def cleanText(text, anchor, Title_ID_All, Redirect_Hashmap, In_Link_Graph_Degree, min_degree, english27):
    """
    Changed by Ehsan Sherkat
    Transforms wiki markup. If the command line flag --escapedoc is set then the text is also escaped
    @see https://www.mediawiki.org/wiki/Help:Formatting
    """
    # Drop transclusions (template, parser functions)
    ...
    #############################################
    if english27 == True:
        text = unicodedata.normalize('NFKD', text).encode('ascii', 'ignore')  # Change none English alphabet to English one (e.g. inför to infor)
        text = numberToLiteral(text)

    # replace internal links
    if anchor:
        text = replaceInternalLinksByAnchor(text, Title_ID_All, Redirect_Hashmap, In_Link_Graph_Degree, min_degree)
    else:
        text = replaceInternalLinks(text)
...
def replaceInternalLinksByAnchor(text, Title_ID_All, Redirect_Hashmap, In_Link_Graph_Degree, min_degree):
    """
    Changed by Ehsan Sherkat
    Replaces internal links of the form:
    [[title |...|label]]trail
    ...
    for s, e in findBalanced(text):
        ...
        res += text[cur:s] + makeInternalLinkByAnchor(title, label, Title_ID_All, Redirect_Hashmap, In_Link_Graph_Degree, min_degree) + trail
        cur = end
    return res + text[cur:]

def makeInternalLinkByAnchor(title, label, Title_ID_All, Redirect_Hashmap, In_Link_Graph_Degree, min_degree):
    """
    Changed by Ehsan Sherkat
    @param anchor:
    @param surface:
    @param Title_ID_All:
    @param Redirect_Hashmap:
    @param In_Link_Graph_Degree:
    @return:
    """
    anchor = ""
    ...
    if Title_ID_All.has_key(title):
        temp_id = Title_ID_All[title]
        if Redirect_Hashmap.has_key(temp_id):
            anchor_id = Redirect_Hashmap[temp_id]
        else:
            anchor_id = temp_id

    if In_Link_Graph_Degree.has_key(anchor_id):
        if int(In_Link_Graph_Degree[anchor_id]) >= min_degree:
            anchor = anchor_id
...
def getAnchor(text):
    ...
    for s, e in findBalanced(text):
        ...
        anchor = makeInternalLinkAnchor(title, label)
        surface = makeInternalLink(title, label)
        if anchor != None:
            anchors.append(anchor)
            if surface != None:
                anchorSurface.append(anchor + "@@" + surface)

    return anchors, anchorSurface

def makeInternalLinkAnchor(title, label):
    """
    Ehsan Sherkat
    @param title:
    @param label:
